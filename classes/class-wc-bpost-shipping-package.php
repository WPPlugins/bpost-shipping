...
class WC_BPost_Shipping_Package {
	...
	public function calculate_shipping() {
		...
		if ( $this->options->is_free_shipping(
			$country_iso_2,
			$this->cart->subtotal,
			$this->cart->applied_coupons
		)
		) {
			return 0;
		}

		return $this->get_minimum_shipping_cost_from_weight( $country_iso_2 );
	}
	 ...
	 */
	private function get_minimum_shipping_cost_from_weight( $country_iso_2 ) {
		// original unit to grams: provides data to \Bpost\BpostApiClient\Bpost\ProductConfiguration\Price::getPriceByWeight;
		$cartWeightGrams = (int) wc_get_weight( $this->get_weight_from_package(), 'g' );

		return $this->product_configuration->get_minimum_shipping_cost_from_weight( $country_iso_2, $cartWeightGrams );
	}
