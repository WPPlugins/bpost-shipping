...
class WC_BPost_Shipping_Shm_Callback_Controller extends WC_BPost_Shipping_Controller_Base {
		...
		switch ( $this->result ) {
			case self::RESULT_SHM_CALLBACK_CONFIRM:
				$shm_saver = new WC_BPost_Shipping_SHM_Saver(
					new WC_BPost_Shipping_Adapter_Shm_Callback( new WC_BPost_Shipping_Container_Postalcode(),
						array_map( 'stripslashes_deep', $_POST ) ),
					new WC_BPost_Shipping_Street_Builder( new WC_BPost_Shipping_Street_Solver() ),
					$this->logger
				);

				$this->force_cache_invalidation();
	...
	/**
	 * We set a "unique" street to have another package hash, and force the shipping calculation
	 * @see \WC_Shipping::calculate_shipping_for_package
	 */
	private function force_cache_invalidation() {
		WC()->customer->set_shipping_address( 'Cache invalidation' );
