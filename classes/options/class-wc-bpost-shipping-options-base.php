...
class WC_BPost_Shipping_Options_Base {
	...
	public function get_api_url() {
		return $this->get_option( 'api_url' ) ?: \Bpost\BpostApiClient\Bpost::API_URL;
	}
	...
	private function migrate_logs_debug_mode() {
		$old_key = 'logs_errors_only';
		$new_key = 'logs_debug_mode';

		if ( $this->has_option( $old_key ) ) {
			if ( $this->get_option( $old_key ) !== 'no' ) {
				$this->options[ $new_key ] = 'yes';
			} else {
				$this->options[ $new_key ] = 'no';
			}
			unset( $this->options[ $old_key ] );
